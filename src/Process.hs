...
import Units (Quantity, Seconds, cubicMeters, items)
import Prelude hiding (Alt (..))


data ProcessSet = ProcessSet
    { main :: Process
    , alt :: [Process]
    }
    deriving (Show)
...
newtype Output a = Output Item
    deriving (Eq, Ord)


newtype Input a = Input Item
    deriving (Eq, Ord)


data ProcessType = Main Item | Alt Item
    deriving (Eq, Ord)


type ProcessIxs = '[Output Process, Input Process, ProcessType]


instance Indexable ProcessIxs Process where
    indices =
        Ix.ixList
...
findProcessSet :: Item -> ProcessSet
findProcessSet i =
    case maybeMain of
...
processes :: IxSet ProcessIxs Process
processes =
    Ix.fromList $
        ( ( \(r, crafter) ->
                Process
